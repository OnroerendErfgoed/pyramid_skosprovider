...
class TestQueryBuilder:
    ...
    def test_build_label_dfs_with_star(self):
        request = self._get_dummy_request(
            {'mode': 'dijitFilteringSelect', 'label': 'Di*'}
        )
        qb = self._get_fut(request)
        q = qb()
        assert isinstance(q, dict)
    ...
    def test_build_matches(self):
        request = self._get_dummy_request({'match': 'https://thingy.org/thing'})
        qb = self._get_fut(request)
        q = qb()
        assert isinstance(q, dict)
        assert 'matches' in q
    ...
    def test_build_matches_broader(self):
        request = self._get_dummy_request(
            {'match': 'https://thingy.org/thing', 'match_type': 'exact'}
        )
        qb = self._get_fut(request)
        q = qb()
        assert isinstance(q, dict)
        assert 'matches' in q
        assert q['matches'] == {'uri': 'https://thingy.org/thing', 'type': 'exact'}
...
class TestRangeHeaders(unittest.TestCase):
    def test_parse_range_header(self):
        ...
        headers = [
            {'header': 'items=0-19', 'result': {'start': 0, 'number': 20, 'finish': 19}},
            {'header': 'items:0-19', 'result': False},
            {'header': 'test', 'result': False},
            {'header': 'items=t-t', 'result': False},
            {'header': 'items=10-0', 'result': {'start': 10, 'finish': 10, 'number': 1}},
        ]
