...
class ProviderView(RestView):
    ...
    def get_uri(self):
        ...
        uri_context['concept_scheme'] = {'@id': 'skos:inScheme', '@type': '@id'}
        if provider:
        ...
        return {
            ...
            'concept_scheme': {
                ...
                'id': self.skos_registry.get_provider(
    ...
    def get_conceptschemes(self):
        ...
        context['subject'] = {'@id': 'dct:subject', '@type': '@id'}
        conceptschemes = []
    ...
    def get_conceptscheme(self):
        ...
        return {
            'id': provider.get_vocabulary_id(),
            'uri': provider.concept_scheme.uri,
            'label': provider.concept_scheme.label(language).label
            if provider.concept_scheme.label(language)
            else None,
            'subject': provider.metadata['subject']
            if provider.metadata['subject']
            else [],
    ...
    def get_concepts(self):
        ...
        if qb.no_result:
            ...
            # Flatten it all
            concepts = list(
                itertools.chain.from_iterable([c['concepts'] for c in concepts])
            )
    ...
    def _page_results(self, concepts):
        ...
        self.request.response.headers['Content-Range'] = 'items %d-%d/%d' % (
            paging_data['start'],
            paging_data['finish'],
            count,
        )
